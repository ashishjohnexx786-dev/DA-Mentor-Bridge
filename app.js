...
const KEY='bridgeMentor2026.fullshell.v1';
const VERSION=8;
...
function exportBackup(){download(`Bridge_Mentor_Backup_${todayKey()}.json`,JSON.stringify(state,null,2),'application/json')}
function exportLessons(){const rows=LESSONS.map(l=>[l.id,l.stageId,l.title,lessonStatus(l.id),ls(l.id).attemptAt||'',ls(l.id).masteredAt||'',ls(l.id).skipped?'Yes':'No']);download('Bridge_Mentor_Lessons.csv',toCSV(['Lesson ID','Stage','Title','Status','Attempt saved','Mastered','Skipped'],rows),'text/csv')}
function exportStudy(){download('Bridge_Mentor_Study_Sessions.csv',toCSV(['Date','Minutes','Seconds','Stage','Lesson','Started','Ended'],state.studyLog.map(x=>[x.date,x.minutes,x.durationSec??'',x.stageId,x.lessonId,x.startedAt??'',x.endedAt??x.at??''])),'text/csv')}
function exportTrainingProof(){const totalMins=state.studyLog.reduce((a,b)=>a+(+b.minutes||0),0),projectLessons=LESSONS.filter(l=>l.formal===false);const rows=[['Total focused study minutes',Math.round(totalMins)],['Formal lessons mastered',FORMAL_LESSONS.filter(l=>ls(l.id).mastered).length],['Formal lessons total',FORMAL_LESSONS.length],['Project phases mastered',projectLessons.filter(l=>ls(l.id).mastered).length],['Project phases total',projectLessons.length],['Protected Gates passed',gatesPassed()],['Protected Gates total',DATA.gates.length],['Evidence items saved',state.evidence.length],['Generated',nowISO()]];download('Bridge_Mentor_Training_Proof.csv',toCSV(['Metric','Value'],rows),'text/csv')}
function exportEvidence(){download('Bridge_Mentor_Evidence.csv',toCSV(['Date','Skill','Type','Strength','Action / validation'],state.evidence.map(x=>[x.createdAt?.slice(0,10),x.skill,x.type,x.strength,x.action])),'text/csv')}
function exportJobs(){download('Bridge_Mentor_Applications.csv',toCSV(['Date','Company','Role','Source','Fit','Status','Mode','Degree wording','Follow-up','Notes'],state.applications.map(x=>[x.date,x.company,x.role,x.source,x.fit,x.status,x.mode,x.degree,x.follow,x.notes])),'text/csv')}
